...
Wrap go-rod with thin type wrappers (`Browser`, `Page`, `Element`, `EvalResult`) that provide a Go-idiomatic API. Use the functional options pattern for configuration. Re-export rod types only where
necessary (e.g., `SelectorType`, `devices.Device`).
