...
| P3 | **CDP-CURRENCY**: evaluate `chromedp/cdproto` for CDP protocol currency — *reference-diff first*; engine adoption REJECTED (ROADMAP 2026-06-18; ADR-0002/0006). Step 1: diff `cdproto` vs `internal/engine/lib/proto/` to surface new/deprecated CDP commands (zero runtime dep), wire into `.scripts/rod-upstream-diff.sh`. Step 2 (gated): adopt `cdproto` as the proto source ONLY if step 1 measures a real coverage gap justifying the `cdp`-client + `proto.*` rewire. | MIT; deps `sysutil`+`easyjson`. chromedp the *engine/client* is NOT adopted — no capability gap, ~127 rod-bound facade methods, stealth is rod-lifecycle-bound. |
| P2 | **DEPRECATION** (removal after 2026-07-02): remove `UserProfile.Cookies/Storage/Headers` (`internal/engine/profile.go`). Superseded by `pkg/scout/vault`. **Phase 1 DONE 2026-06-09** (branch `feat/profile-vault-phase1`): vault gained `CaptureFromPage` + `Handle.ApplyStorageToPage`; `CaptureProfile` no longer captures secrets; `ApplyProfile` is a deprecated no-op; CLI gained `scout vault capture` and `scout profile` is non-secret (cookie values masked in `show`). The 3 fields remain, read only by the `vault set --from-profile` bridge + legacy `profile show`. **Phase 2 (after 2026-07-02):** drop the 3 fields + `ProfileOriginStorage`; remove `FromUserProfile` + `vault set --from-profile`; remove `Save/LoadProfileEncrypted`; strip secret branches from `Merge`/`Diff`/`Validate`/`profile show`; deprecate/remove gRPC `LoadProfile`. This deletes ~11 intentional `staticcheck` SA1019 deprecation-warning sites (legacy `show`/`load` + gRPC stub) left un-annotated in Phase 1 (lint advisory). Also consolidate the duplicated origin helper (`engine.originFromURL` == `vault.originFrom`) and add a debug log for swallowed storage-read errors in `CaptureFromPage`. | Fields marked deprecated 2026-06-02; Phase 1 spec+plan in `docs/superpowers/`. |
...
| P3 | **LINT-01**: re-enable BLOCKING golangci-lint on `main` (currently advisory; CI lint runs on non-main branches only). Blocked by repo-wide debt: **~200 findings** as of 2026-06-10 (`golangci-lint run ./...`) — top linters: contextcheck ~50, godoclint ~29, modernize ~24, errchkjson ~18, staticcheck ~17, unparam ~10; concentrated in `pkg/scout/tools/page.go` (20), `internal/engine/websocket.go`, `cmd/scout/profile.go`, `hacks/*`. **Do this as a dedicated burndown, not a flag flip.** The `usestdlibvars` slice was cleared 2026-06-10 (`dab18e7`, 209→200). **Two "mechanical" linters are NOT auto-fix-safe on this codebase and need manual handling: `modernize`** (its `gson.Int(x)`→`new(x)` rewrite is invalid Go — breaks `lib/devices`, `lib/input`, `engine/page`, `engine/input`; consider a `//nolint:modernize` exclusion for `gson.*` call sites or a targeted manual pass) **and `dupword`** (false positives that changed meaning — deleted a sentence-leading "Crawl" in `aihost/claude/assets.go` skill body, renamed a `"nil nil"` subtest). The **8 SA1019** findings are intentional and must wait for the Profile→Vault Phase 2 removal (after 2026-07-02). | Sequence after 2026-07-02 for the SA1019 part. Use `golangci-lint run --fix --default=none --enable=<safe-linter>` per-linter, verifying `go vet ./...` + `go test -short ./...` after each (autofix can break test-file imports + corrupt content). |
...
- **DONE 2026-06-09 — the `-short` test gate is BLOCKING on `main`** (`c93fe63`; build + `-short` tests block, lint/vulncheck stay advisory). Was advisory through 2026-06-04. Browser suite is fully `-short`-gated. A blocking flip was attempted and reverted: it surfaced **CI-Linux-only failures** — process-spawning reaper / session-doctor acceptance tests (`holder PID still alive after reap`) fail on the GitHub Actions container's process-kill model. Steps taken:
  ...
  - **Result**: full `go test -short ./...` = 0 failures / no timeouts locally AND on the CI Linux runner.
  - **DONE 2026-06-09 — BLOCKING gate landed** (`c93fe63`): the three `TestSessionDoctorAcceptance_*`
    tests (`cmd/scout/session_doctor_acceptance_test.go`) are now `-short`-gated, matching
    `reaper_acceptance_test.go` (gated 2026-06-04). Linux run `27239035810` was green with the Test
    step blocking, so `continue-on-error` was dropped from `test.yml`; `-short` regressions now block `main`.
  - **Remaining (P3)**: a separate Chromium-in-CI job for the full non-`-short` browser suite + a coverage
    threshold; triage the advisory lint.
