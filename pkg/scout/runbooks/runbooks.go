...
// Preset describes an available runbook preset.
type Preset struct {
	ID          string `json:"id"`
...
// Load reads and parses a preset runbook by ID.
func Load(id string) (*runbook.Runbook, error) {
	p, ok := index[id]
	if !ok {
		return nil, fmt.Errorf("runbooks: unknown preset %q", id)
	}

	data, err := presetsFS.ReadFile(p.File)
	if err != nil {
		return nil, fmt.Errorf("runbooks: read %s: %w", p.File, err)
	}
