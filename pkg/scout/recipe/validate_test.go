...
func TestValidateRecipe_Valid(t *testing.T) {
	...
	result, err := ValidateRecipe(b, r)
	if err != nil {
		t.Fatalf("ValidateRecipe failed: %v", err)
	}
	...
	if result.URL != ts.URL+"/recipe-extract" {
		t.Errorf("url = %q, want %q", result.URL, ts.URL+"/recipe-extract")
	}
}

func TestValidateRecipe_Invalid(t *testing.T) {
	ts := newTestServer()
	defer ts.Close()
	b := newTestBrowser(t)
...
func TestValidateRecipe_Automate(t *testing.T) {
	ts := newTestServer()
	defer ts.Close()
	b := newTestBrowser(t)
...
func TestValidateRecipe_NilRecipe(t *testing.T) {
	b := newTestBrowser(t)
	_, err := ValidateRecipe(b, nil)
	if err == nil {
		t.Error("expected error for nil recipe")
	}
}

func TestValidateRecipe_NoURL(t *testing.T) {
	b := newTestBrowser(t)
	r := &Recipe{Version: "1", Name: "no-url", Type: "automate", Steps: []Step{{Action: "click", Selector: "#x"}}}
	_, err := ValidateRecipe(b, r)
	if err == nil {
		t.Error("expected error for recipe with no URL")
	}
}
...
func TestSelectorHealthCheck(t *testing.T) {
	...
	page, err := b.NewPage(ts.URL + "/recipe-extract")
	if err != nil {
