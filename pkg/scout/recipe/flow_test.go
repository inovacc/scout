...
func TestDetectFlow_NilBrowser(t *testing.T) {
	...
	if got := err.Error(); got != "recipe: flow: nil browser" {
...
func TestGenerateFlowRecipe_NoSteps(t *testing.T) {
	_, err := GenerateFlowRecipe(nil, "test")
	if err == nil {
		t.Fatal("expected error for empty steps")
	}
}

func TestGenerateFlowRecipe_SingleListingPage(t *testing.T) {
	steps := []FlowStep{
		{
	...
	r, err := GenerateFlowRecipe(steps, "products")
	if err != nil {
	...
	if r.Items == nil {
		t.Fatal("expected items spec for extract recipe")
	}
}

func TestGenerateFlowRecipe_LoginThenSearch(t *testing.T) {
	steps := []FlowStep{
		{
	...
	r, err := GenerateFlowRecipe(steps, "login-search")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGenerateFlowRecipe_DefaultName(t *testing.T) {
	steps := []FlowStep{
		{URL: "http://example.com", PageType: "unknown"},
	}

	r, err := GenerateFlowRecipe(steps, "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if r.Name != "flow-recipe" {
		t.Errorf("expected default name 'flow-recipe', got %s", r.Name)
	}
}
