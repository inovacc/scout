...
// InteractiveConfig holds the configuration for interactive recipe creation.
...
// InteractiveCreate walks the user through step-by-step recipe creation.
func InteractiveCreate(cfg InteractiveConfig) (*Recipe, error) {
	if cfg.Browser == nil {
		return nil, fmt.Errorf("recipe: interactive: nil browser")
	}
	if cfg.URL == "" {
		return nil, fmt.Errorf("recipe: interactive: empty URL")
	}
	if cfg.Writer == nil {
		return nil, fmt.Errorf("recipe: interactive: nil writer")
	}
	if cfg.Reader == nil {
		return nil, fmt.Errorf("recipe: interactive: nil reader")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("recipe: interactive: analyze: %w", err)
	}
	...
	if len(analysis.Containers) == 0 && len(analysis.Forms) == 0 {
		return nil, fmt.Errorf("recipe: interactive: no containers or forms detected on page")
	}
	...
	if len(analysis.Containers) > 0 {
		...
		if choice != "" {
			for _, f := range strings.Split(choice, ",") {
	...
	name := prompt(w, scanner, "Recipe name", defaultName)
	if name == "" {
		name = defaultName
	}

	// Generate recipe with selected fields.
	if len(selectedFields) > 0 {
		genOpts = append(genOpts, WithGenerateFields(selectedFields...))
	}

	r, err := GenerateRecipe(analysis, genOpts...)
	if err != nil {
		return nil, fmt.Errorf("recipe: interactive: generate: %w", err)
	}
	r.Name = name
