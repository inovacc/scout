...
func TestGenerateRecipe_Extract(t *testing.T) {
	ts := newAnalyzeServer(t)
	defer ts.Close()
	b := newTestBrowser(t)
	...
	r, err := GenerateRecipe(analysis)
	if err != nil {
		t.Fatalf("GenerateRecipe failed: %v", err)
	}
...
func TestGenerateRecipe_Automate(t *testing.T) {
	ts := newAnalyzeServer(t)
	defer ts.Close()
	b := newTestBrowser(t)
...
func TestGenerateRecipe_ForceType(t *testing.T) {
	ts := newAnalyzeServer(t)
	defer ts.Close()
	b := newTestBrowser(t)
	...
	_, err = GenerateRecipe(analysis, WithGenerateType("extract"))
	if err == nil {
	...
	_, err = GenerateRecipe(analysis2, WithGenerateType("automate"))
	if err == nil {
		t.Error("expected error when forcing automate on page without forms")
	}
}

func TestGenerateRecipe_Validate(t *testing.T) {
	ts := newAnalyzeServer(t)
	defer ts.Close()
	b := newTestBrowser(t)
	...
	if err := r.Validate(); err != nil {
		t.Errorf("generated recipe failed validation: %v", err)
	}
}

func TestGenerateRecipe_RunEndToEnd(t *testing.T) {
	ts := newAnalyzeServer(t)
	defer ts.Close()
	b := newTestBrowser(t)
...
func TestGenerateRecipe_NilAnalysis(t *testing.T) {
	_, err := GenerateRecipe(nil)
	if err == nil {
