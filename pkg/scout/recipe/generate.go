...
// GenerateOption configures GenerateRecipe behavior.
...
// WithGenerateType forces the recipe type ("extract" or "automate").
func WithGenerateType(t string) GenerateOption {
	return func(o *generateOptions) { o.forceType = t }
}

// WithGenerateFields specifies which fields to include in an extract recipe.
...
// GenerateRecipe creates a Recipe from a SiteAnalysis.
func GenerateRecipe(analysis *SiteAnalysis, opts ...GenerateOption) (*Recipe, error) {
	if analysis == nil {
		return nil, fmt.Errorf("recipe: generate: nil analysis")
	}
	...
	recipeType := detectRecipeType(analysis)
	if o.forceType != "" {
		recipeType = o.forceType
	}
...
func generateExtract(a *SiteAnalysis, o *generateOptions, name string) (*Recipe, error) {
	if len(a.Containers) == 0 {
		return nil, fmt.Errorf("recipe: generate: no containers found for extract recipe")
	}
	...
	if len(fields) == 0 {
		return nil, fmt.Errorf("recipe: generate: no fields discovered in top container")
	}
...
func generateAutomate(a *SiteAnalysis, _ *generateOptions, name string) (*Recipe, error) {
	if len(a.Forms) == 0 {
		return nil, fmt.Errorf("recipe: generate: no forms found for automate recipe")
	}
