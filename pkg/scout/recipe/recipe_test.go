...
// newTestServer creates an httptest server with recipe test fixture routes.
func newTestServer() *httptest.Server {
	mux := http.NewServeMux()

	mux.HandleFunc("/recipe-extract", func(w http.ResponseWriter, _ *http.Request) {
	...
	mux.HandleFunc("/recipe-extract-page2", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><body>
	...
	mux.HandleFunc("/recipe-paginated", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><body>
			<div class="item"><h2>Page1 Item</h2></div>
			<a class="next" href="/recipe-paginated-2">Next</a>
		</body></html>`)
	})

	mux.HandleFunc("/recipe-paginated-2", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><body>
	...
	mux.HandleFunc("/recipe-dedup", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><body>
	...
	mux.HandleFunc("/recipe-auto", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><body>
	...
	mux.HandleFunc("/recipe-wait", func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, `<html><body>
...
func TestLoadFile_NotFound(t *testing.T) {
	_, err := LoadFile("/nonexistent/recipe.json")
	if err == nil {
...
func TestRun_UnknownType(t *testing.T) {
	r := &Recipe{Version: "1", Name: "test", Type: "bogus"}
	_, err := Run(context.Background(), nil, r)
	if err == nil {
		t.Error("expected error for unknown type")
	}
}
...
func TestRunAutomate_NavigateTypeClick(t *testing.T) {
	...
	r := &Recipe{
		...
		Steps: []Step{
			{Action: "navigate", URL: ts.URL + "/recipe-auto"},
...
func TestRunAutomate_Wait(t *testing.T) {
	...
	_, err := Run(context.Background(), b, r)
	if err != nil {
		t.Fatalf("Run failed: %v", err)
	}
}
...
func TestRunAutomate_NavigateExistingPage(t *testing.T) {
	...
	r := &Recipe{
		...
		Steps: []Step{
			{Action: "navigate", URL: ts.URL + "/recipe-auto"},
			{Action: "navigate", URL: ts.URL + "/recipe-wait"},
...
func TestRun_AutomateEndToEnd(t *testing.T) {
	...
	data := fmt.Sprintf(`{
		...
		"type": "automate",
		"steps": [
			{"action": "navigate", "url": "%s/recipe-auto"},
			{"action": "click", "selector": "#btn"},
			{"action": "eval", "script": "() => document.getElementById('result').textContent", "as": "result"}
		]
	}`, ts.URL)
