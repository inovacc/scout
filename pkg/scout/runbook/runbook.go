...
// Runbook defines a declarative scraping or automation playbook.
type Runbook struct {
...
// Step is a single action in an automation runbook.
type Step struct {
...
// Result holds the output of a runbook execution.
type Result struct {
...
// LoadFile reads and parses a runbook JSON file.
...
// Parse decodes a runbook from JSON bytes.
...
// Apply executes a runbook against the given browser.
func Apply(ctx context.Context, browser *scout.Browser, r *Runbook) (*Result, error) {
	switch r.Type {
	case "extract":
...
func (r *Runbook) resolveAllSelectors() error {
