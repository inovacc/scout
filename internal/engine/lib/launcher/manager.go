...
import (
	...
	cdp2 "github.com/inovacc/scout/internal/engine/lib/cdp"
...
func MustNewManaged(serviceURL string) *Launcher {
	...
	// upstream limitation: latest Chromium in Docker crashes on HTTP/2; disable until
	// the upstream Chromium issue is resolved.
	l.Set("disable-http2")
...
func (l *Launcher) MustClient() *cdp2.Client {
	u, h := l.ClientHeader()
	return cdp2.MustStartWithURL(l.ctx, u, h)
}

// Client for launching browser remotely via the launcher.Manager.
func (l *Launcher) Client() (*cdp2.Client, error) {
	u, h := l.ClientHeader()
	return cdp2.StartWithURL(l.ctx, u, h)
}
