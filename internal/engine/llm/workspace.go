...
// Session tracks a named workspace for LLM jobs.
type Session struct {
	ID        string            `json:"id"`
...
// Workspace manages session and job state on the filesystem.
//
// Directory structure:
//
//	<root>/
...
// NewWorkspace creates or opens a workspace at the given path.
...
func (w *Workspace) CreateSession(name string, meta map[string]string) (*Session, error) {
...
func (w *Workspace) GetSession(id string) (*Session, error) {
	idx, err := w.readSessionIndex()
	if err != nil {
		return nil, err
	}
...
func (w *Workspace) ListSessions() ([]Session, error) {
	idx, err := w.readSessionIndex()
	if err != nil {
		return nil, err
	}
...
func (w *Workspace) CurrentSession() (*Session, error) {
	idx, err := w.readSessionIndex()
	if err != nil {
		return nil, err
	}
...
func (w *Workspace) SetCurrentSession(id string) error {
	idx, err := w.readSessionIndex()
	if err != nil {
		return err
	}
...
func (w *Workspace) CreateJob(sessionID, url, prompt string, meta map[string]string) (*Job, error) {
...
func (w *Workspace) UpdateJob(job *Job) error {
...
func (w *Workspace) GetJob(id string) (*Job, error) {
...
func (w *Workspace) ListJobs() ([]JobRef, error) {
	idx, err := w.readJobIndex()
	if err != nil {
		return nil, err
	}
...
func (w *Workspace) CurrentJob() (*Job, error) {
	idx, err := w.readJobIndex()
	if err != nil {
		return nil, err
	}
...
func (w *Workspace) ListSessionJobs(sessionID string) ([]JobRef, error) {
	idx, err := w.readJobIndex()
	if err != nil {
		return nil, err
	}
...
func (w *Workspace) sessionsPath() string {
	return filepath.Join(w.root, "sessions.json")
}

func (w *Workspace) jobsIndexPath() string {
	return filepath.Join(w.root, "jobs", "jobs.json")
}

func (w *Workspace) readSessionIndex() (*SessionIndex, error) {
...
func (w *Workspace) readJobIndex() (*JobIndex, error) {
	var idx JobIndex
...
func (w *Workspace) readJSON(path string, v any) error {
...
func (w *Workspace) writeJSON(path string, v any) error {
