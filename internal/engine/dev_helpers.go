...
import (
	...
	"github.com/inovacc/scout/internal/engine/lib/assets"
...
func (b *rodBrowser) ServeMonitor(host string) string {
	...
	mux.HandleFunc("/api/pages", func(w http.ResponseWriter, _ *http.Request) {
		res, err := proto2.TargetGetTargets{}.Call(b) //nolint: contextcheck
		utils.E(err)

		list := []*proto2.TargetTargetInfo{}

		for _, info := range res.TargetInfos {
			if info.Type == proto2.TargetTargetInfoTypePage {
	...
	mux.HandleFunc("/api/page/", func(w http.ResponseWriter, r *http.Request) {
		...
		info, err := b.pageInfo(proto2.TargetTargetID(id)) //nolint: contextcheck
		utils.E(err)
		w.WriteHeader(http.StatusOK)
	...
	mux.HandleFunc("/screenshot/", func(w http.ResponseWriter, r *http.Request) {
		id := r.URL.Path[strings.LastIndex(r.URL.Path, "/")+1:]
		target := proto2.TargetTargetID(id)
...
func (b *rodBrowser) trySlowMotion() {
	if b.slowMotion == 0 {
		return
	}
...
func (p *rodPage) ExposeHelpers(list ...*js2.Function) {
	p.MustEvaluate(evalHelper(&js2.Function{
...
func (p *rodPage) Overlay(left, top, width, height float64, msg string) (remove func()) {
	id := utils.RandString(8)

	_, _ = p.root.Evaluate(evalHelper(js2.Overlay,
		id,
		left,
		top,
		width,
	...
	remove = func() {
		_, _ = p.root.Evaluate(evalHelper(js2.RemoveOverlay, id))
	}

	return
}

func (p *rodPage) tryTrace(typ TraceType, msg ...any) func() {
	if !p.browser.trace {
		return func() {}
	}
...
func (p *rodPage) tryTraceQuery(opts *EvalOptions) func() {
	if !p.browser.trace {
		return func() {}
	}
...
func (p *rodPage) tryTraceReq(includes, excludes []string) func(map[proto2.NetworkRequestID]string) {
	if !p.browser.trace {
		return func(map[proto2.NetworkRequestID]string) {}
	}
	...
	update := func(list map[proto2.NetworkRequestID]string) {
...
func (el *rodElement) Overlay(msg string) (removeOverlay func()) {
	id := utils.RandString(8)

	_, _ = el.Evaluate(evalHelper(js2.ElementOverlay,
		id,
		msg,
	).ByPromise())

	removeOverlay = func() {
		_, _ = el.Evaluate(evalHelper(js2.RemoveOverlay, id))
	}

	return
}

func (el *rodElement) tryTrace(typ TraceType, msg ...any) func() {
...
func (m *Mouse) initMouseTracer() {
	_, _ = m.page.Evaluate(evalHelper(js2.InitMouseTracer, m.id, assets.MousePointer).ByPromise())
}

func (m *Mouse) updateMouseTracer() bool {
	res, err := m.page.Evaluate(evalHelper(js2.UpdateMouseTracer, m.id, m.pos.X, m.pos.Y))
	if err != nil {
		return true
	}
