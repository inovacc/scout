...
Scout is a same-machine-only Go browser automation library with the core engine in `internal/engine/` (internalized rod fork) and a public facade at `pkg/scout/`. A unified Cobra CLI (`cmd/scout/`) exposes all features. Sessions are per-command (no daemon); multi-step automation is done via `scout repl`, the stdio MCP server, or `strategy`/`runbook` files.

## Build & Test

Uses Taskfile. Key commands: `task build`, `task test`, `task test:unit`, `task check`, `task lint`, `task lint:fix`, `task fmt`, `task vet`, `task generate:stealth`.
...
pkg/scout/mcp/                MCP server (built-in tools + WS tools, 3 resources, stdio transport only)
...
cmd/scout/                    Unified Cobra CLI (gops agent, logger)
examples/                     18 runnable examples (simple/ and advanced/)
```

Import: `github.com/inovacc/scout/pkg/scout`. Public facade re-exports `internal/engine` types. Core does NOT import Cobra. Sessions are per-command (no daemon); the MCP server is stdio-only.
...
- **Platform-specific**: `WithXvfb()` in `option_unix.go`.
...
- **Interaction capture**: opt-in (`scout interactions on/off/status/list` or `SCOUT_INTERACTIONS=1`, off by default) records a redacted, AI-friendly JSONL trace of CLI invocations, MCP tool calls, and REPL browser actions to `<scouthome>/captures/<id>.jsonl` (files `0o600`). `internal/interaction` is the recorder: per-session `Recorder` (append-only JSONL, `session_start`/`session_end` framing, monotonic `seq`) plus a process-global default with nil-safe `Init`/`Emit`/`Close`. `internal/redact` is the shared secret redactor (`Args`/`Map`/`URL`/`Header`/`Body`, substring-based by design — over-redacts rather than miss `access_token`), also used by the logger. Emit hooks live at existing chokepoints: root `PersistentPreRunE` (`cli`), `addTracedTool` (`mcp_tool`), REPL dispatch (`browser_action`). v2/v3 (network/console hooks, rotation/retention) are deferred — see `docs/superpowers/specs/2026-06-14-interaction-capture-design.md`.
...
- **Session startup cleanup**: `CleanStaleSessions()` runs in `main()` on every invocation. Uses a single-shot `os.RemoveAll` for the fast path; persistent failures (Windows AV / Search Indexer / OneDrive holding Chrome SQLite + LevelDB files) are enqueued for `StartCleanupRetrier` which retries every 60 s for the process lifetime. Removes non-reusable sessions unconditionally and orphaned dirs without `scout.pid`. **Reusable sessions are auto-cleaned only when `IsExpired()` trips** (H6 + bounded-lifetime); otherwise only manual `scout session reset` (or `rm`) removes them.
- **Session monitors**: `monitors.json` sidecar persists per-session monitor config (HAR/hijack/console/ws sinks + block rules). Written when monitoring is enabled for a command; read on teardown to flush the HAR artifact to the configured path (default `<session>/har.json`).
...
- **OpenTelemetry tracing**: `internal/tracing/` package. No-op by default unless `SCOUT_TRACE=1` or `OTEL_EXPORTER_OTLP_ENDPOINT` is set (dormant; slated for a follow-up). `tracing.Init(ctx, Config{})` in CLI bootstrap. MCP tools auto-instrumented via `addTracedTool()` wrapper in `pkg/scout/mcp/server.go`. Scraper CLI uses `ScraperSpan()`. Custom spans: `tracing.Start(ctx, "name", attrs...)`.
...
- **MCP tool count**: built-in tools after deprecation cleanup (navigate, click, type, extract, eval, back, forward, wait, screenshot, snapshot, pdf, session_list, session_reset, open, ws_listen, ws_send, ws_connections). Many tools migrated to plugins.
- **Shared command executor**: all REPL + MCP browser commands route through `pkg/scout/tools/` verbs; adding a capability = one `tools/` verb + one REPL case + one MCP `AddTool`. A `tools.TestVerbParity` manifest guards parity.
- **Touch emulation**: `WithTouchEmulation()` for desktop touch simulation; `Page.Touch()`, `Page.Swipe()`, `Page.PinchZoom()` for touch gestures. Status: the `WithMobile(MobileConfig{})` library option (ADB-connected Android Chrome) remains but is dormant, slated for a follow-up; the `scout mobile` CLI was removed.
...
- **Claude Code plugin**: `.claude-plugin/plugin.json` + `.mcp.json` + 6 skills + 5 agents (site-tester, web-scraper, site-mapper, session-capture, flow-porter). Test: `claude --plugin-dir .`. Skills: `/scout:scrape`, `/scout:screenshot`, `/scout:test-site`, `/scout:gather`, `/scout:crawl`, `/scout:monitor`. Validate: `task plugin:validate`.
...
- **Benchmarks**: `go test -bench=. ./internal/engine/hijack/ ./internal/metrics/` — benchmarks for hot paths.
...
- **Flow capture→replay**: `pkg/scout/flow` records a browser flow (`scout flow capture`), an LLM analyzer turns it into a reviewed `flow.yaml` (`scout flow analyze`), and a deterministic runtime replays the REST/GraphQL calls with no browser (`scout flow run`), chaining values via `${var}`/`${secret.NAME}` (secrets resolved from a `pkg/scout/vault` profile at send time, never embedded). `scout flow verify --golden` diffs status parity vs the golden `capture.json`. The reviewed spec is the deterministic contract; the LLM runs only at analyze time. Security: capture artifacts and generated specs are written `0o600`; the analyzer redacts secret values from its LLM digest (response bodies, URL query strings, sensitive headers) and parameterizes secret-bearing headers + URL-query tokens in the emitted spec to `${secret.*}` so a generated `flow.yaml` is secret-free (enforced by `pkg/scout/flow/hygiene_test.go`). v1 covers REST/JSON + GraphQL and emits header chains; the browser capture driver is behind `//go:build integration`.
...
Scout is a same-machine-only Go browser automation library with an internalized rod fork, public facade API, stdio MCP server, plugin system, REPL, and a Cobra CLI. It provides headless and headed browser control for scraping, testing, and monitoring. Sessions are per-command (no daemon).
...
- Cobra v1.10.2 (`github.com/spf13/cobra`) - CLI framework in `cmd/scout/`
...
- `go.opentelemetry.io/otel` v1.43.0 - OpenTelemetry tracing (dormant, no-op by default)
...
- `SCOUT_HOME` - Override per-user state root (default: `%LOCALAPPDATA%\Scout` on Windows, `~/Library/Application Support/Scout` on macOS, `$XDG_DATA_HOME/scout` or `~/.local/share/scout` on Linux). Honored by every persisted directory (sessions, browsers, plugins, fingerprints, extensions, reports, electron cache).
...
- `~/.scout/` - Session data, fingerprints, plugins, reports
...
- Generated files: `scout.go` facade (via `gen-facade-full.go`)
- Lowercase, single-word where possible: `engine`, `browser`, `stealth`, `hijack`
...
- Package-specific exclusions: `forbidigo`/`gocritic` excluded for `examples/`, `contextcheck` excluded for `pkg/scout/scraper/`
...
- Session management with process-level tracking via gops; sessions are per-command (no daemon)
- Lazy browser initialization in the MCP server
## Layers
- Purpose: Cobra-based CLI exposing all Scout features
- Location: `cmd/scout/`
- Contains: One file per command group (e.g., `scrape.go`, `crawl.go`, `gather.go`, `hijack.go`, `repl.go`)
- Depends on: `pkg/scout`, `pkg/scout/mcp`, `pkg/scout/scraper`, `internal/engine/session`, `internal/flags`, `internal/logger`, `internal/tracing`
...
- Used by: All entry points (CLI, MCP, plugins)
...
- Depends on: `internal/engine/lib/` (internalized rod), `internal/engine/browser/`, `internal/engine/stealth/`, `internal/engine/fingerprint/`, `internal/engine/hijack/`, `internal/engine/llm/`, `internal/engine/session/`
...
- Purpose: Model Context Protocol server (stdio only) exposing browser tools for AI assistants
- Location: `pkg/scout/mcp/`
- Contains: Tool definitions split by category: `tools_browser.go` (navigate, click, type, extract, eval, etc.), `tools_capture.go` (screenshot, snapshot, pdf), `tools_session.go` (session management), `tools_websocket.go` (WS monitoring), `resources.go` (3 MCP resources)
...
- Used by: CLI `cmd/scout/` (stdio transport only)
...
- Triggers: AI assistant connects via stdio (stdio transport only)
- Responsibilities: browser tools, 3 resources, lazy browser lifecycle, idle timeout, OpenTelemetry tracing
